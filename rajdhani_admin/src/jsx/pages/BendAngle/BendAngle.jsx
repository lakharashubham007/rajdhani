...
import { addFittingDashSizeApi, deleteFittingDashSizeApi, GetEditFittingDashSizeData, 
    getFittingDashSizeApi, UpdateFittingDashSize, UpdateFittingDashSizeStatus } from "../../../services/apis/FittingDashSize";
import { addBendAngleApi, deleteBendAngleApi, getBendAngleApi, GetEditBendAngleData, UpdateBendAngle, UpdateBendAngleStatus } from "../../../services/apis/BendAngle";
...
const BendAngle = () => {
  ...
  const [bendAngleList, setBendAngleList] = useState([]);
  ...
  //   getSubCategoriesApi
  const fetchBendAngle = async (sortValue) => {
    // Set loading to true when the API call starts
    setLoading(true);
    try {
      const res = await getBendAngleApi(
        currentPage,
        sort,
        sortValue,
        searchInputValue
      );
      setBendAngleList(res?.data?.bendAngles);
  ...
  const handleEditBendAngle = async (id) => {
    try {
      const res = await GetEditBendAngleData(id);
      if (res?.data?.success) {
        const data = res?.data?.bendAngle;
  ...
  const handleDeleteSubmit = async () => {
    try {
      const res = await deleteBendAngleApi(deleteTableDataId);
      if (res.status === 200) {
  ...
  const handleStatusChange = async (partId, currentStatus) => {
    ...
    try {
      const res = await UpdateBendAngleStatus(partId, fdata);
      if (res.status === 200) {
        Toaster.success(res?.data?.message); // Display success message
        fetchBendAngle();
      } else {
  ...
  const handleUpdateSubmit = async () => {
    try {
      const res = await UpdateBendAngle(editPartId, formData);
      if (res.status === 200) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (isEdit) {
      ...
      setLoading(true);
      try {
        const res = await addBendAngleApi(formData);
        if(res?.data?.success) {
          setLoading(false);
          Toaster.success(res?.data?.message);
              ...
              <div className="table-responsive">
                <div id="holidayList" className="dataTables_wrapper no-footer">
                  ...
                  >
                    ...
                    <tbody>
                      {bendAngleList?.bendAngles?.map((data, ind) => (
                        <tr key={ind}>
                          <td>
                            {/* <strong>{ind + 1}</strong> */}
                            <strong>
                          ...
                          <td>
                            <button
                              ...
                              onClick={() => handleEditBendAngle(data?._id)}
                            >
                  ...
                  <div>
                    ...
                    <div className="d-sm-flex text-center justify-content-end align-items-center mt-3">
                      <div className="pagination-container">
                        <ReactPaginate
                          pageCount={Math.ceil(
                            bendAngleList?.totalFittingThreads / bendAngleList?.rowsPerPage
                          )}
