...
import { updateItemQuantityWithLogsInInventoryApi } from "../../../../services/apis/InventoryItemLogsApi";
...
const theadData = [
  ...
  { heading: "Deliverable", sortingVale: "isDeliverable" },
...
const AssignQuantityForSaleOrder = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const payload = rows?.flatMap((item) => {
        ...
        const logs = [];
        if (item.assign_quantity || item.reserve_quantity) {
          logs.push({
            ...item,
          });
        }
      ...
      // ✅ Success alert after API response
      if (res) {
        Swal.fire({
          icon: "success",
                ...
                <div className="row">
                  ...
                  <div className="col-sm-4">
                    <h2 className="header-title" style={{ display: 'inline', whiteSpace: 'nowrap' }}>
                      Rajdhani - Assign and Reserve Quantity
                    </h2>
                  </div>
                </div>
              ...
              <Col lg={12}>
                <div className="">
                  <div className="card-header pb-0 px-0">
                    <h4 className="card-title">Sale Order Items</h4>
                    <div className="mb-3">
                              ...
                              <tr key={row.id}
                                ...
                                <td style={{ minWidth: '220px' }}>
                                  {Number(row?.available_quantity) > 0 ? (() => {
                                    ...
                                    const canFulfill = Math.min(qty, avail);
                                    const short = Math.max(qty - avail, 0);
                                    const pct = qty > 0 ? (canFulfill / qty) * 100 : 0;

                                    // traffic-light color
                                    const fillColor =
                                      pct === 100 ? "#4caf50" : pct >= 50 ? "#ff9800" : "#f44336";

                                    return (
                                      <div className="progress-indicator">
                                        <div className="progress-label" style={{ marginBottom: 6 }}>
                                          {/* Keep your original numbers visible */}
                                          {avail}{" "}
                                          {short > 0 && (
                                        ...
                                        <div
                                          ...
                                          aria-label="Inventory fulfillment progress"
                                          role="progressbar"
                                          aria-valuenow={Math.round(pct)}
                                          aria-valuemin={0}
                                          aria-valuemax={100}
                                          title={`${canFulfill}/${qty} can be fulfilled from inventory`}
                                        >
                                          <div
                                ...
                                <td style={{ display: 'flex', justifyContent: 'center' }}>
                                  <div
                                    onClick={(e) => {
                                      e.stopPropagation(); // ✅ prevent default checkbox toggle
                                      handleSelect(row._id);

                                      handleChangeRow(
                                        index,
                                        "isDeliverable",
                                        !row.isDeliverable // toggle value true/false
                                      );
                                    ...
                                    style={{
                                      ...
                                      backgroundColor: selectedIds.includes(row._id) ? "#4CAF50" : "#fff",
                                    }}
                                  >
        ...
        <Modal.Body>
          {selectedRowData && (
            ...
            >
              ...
              {/* avalable quantity Indicator */}
              <div style={{ minWidth: '220px', marginTop: '15px' }}>
                {Number(selectedRowData?.available_quantity) > 0 ? (() => {
                  const qty = Number(selectedRowData?.quantity) || 0;
                  const avail = Number(selectedRowData?.available_quantity) || 0;
                  const canFulfill = Math.min(qty, avail);
                  const short = Math.max(qty - avail, 0);
                  const pct = qty > 0 ? (canFulfill / qty) * 100 : 0;
                  ...
                  return (
                    <div className="progress-indicator">
                      <div className="progress-label" style={{ marginBottom: 6 }}>
                        {/* Keep your original numbers visible */}
                        Ordered Quanity: {qty} / Available Quantity: {avail}{" "}
                        {short > 0 && (
                          <span style={{ color: "#d32f2f", fontWeight: 600 }}>
                            (short by {short})
                          </span>
              ...
              >
                ...
                {/* Assign quantity */}
                <div>
                  <label style={{ color: 'black', margin: '5px' }}>Assign Quantity</label>
                  <input
                    type="number"
                    placeholder="Assign Quantity"
                    value={selectedRowData.assign_quantity}
                    onChange={(e) =>
                      handleChangeRow(
                        selectedRowIndex,
                        "assign_quantity",
                ...
                {/* Reserve quantity */}
                <div>
                  <label style={{ color: 'black', margin: '5px' }}>Reserve Quantity</label>
                  <input
                    type="number"
                    placeholder="Reserve Quantity"
                    min="0"
                    value={selectedRowData.reserve_quantity}
                    onChange={(e) =>
                      handleChangeRow(
                        selectedRowIndex,
                        "reserve_quantity",
...
export default AssignQuantityForSaleOrder;
