...
const theadData = [
  ...
  { heading: "SubCategory Id", sortingVale: "_id" },
  { heading: "SubCategory Name", sortingVale: "name" },
...
//   getSubCategoriesApi
  const fetchSubCategories=async(sortValue)=>{
        ...
        try {
          ...
          setCategoryList(res.data?.subcategories);
          setOriginalCategoryList(res.data?.subcategories);
  ...
  const fetchAllCategoryList=async()=>{
    ...
    const dropdownCategory = res?.data?.categories?.map(
      (category) => ({
        value: category._id,
        label: category.name,
      })
 ...
 useEffect(()=>{
  ...
  const handleUpdateSubmit = async () => {
    try {
      const res = await UpdateSubCategoryApi(editCategoryId, formData);
      if (res.status === 200) {
  ...
  function SotingData(name, ind) {
    const sortedPeople = [...categoryList?.subcategories];

    if (iconData.complete) {
      const sortValue = { value: name, type: "asc" };
      fetchSubCategories(sortValue);
    } else {
  ...
  const handleDeleteSubmit = async () => {
    try {
      const res = await deleteSubCategoriesApi(deleteTableDataId);
              ...
              <div className="table-responsive">
                <div id="holidayList" className="dataTables_wrapper no-footer">
                  ...
                  >
                    ...
                    <tbody>
                      {categoryList?.subcategories?.map((data, ind) => (
                        <tr key={ind}>
                          <td>
                            <strong>{ind + 1}</strong>
                          </td>
                          ...
                          <td>
                            <button
                              ...
                              onClick={() => handleEditSubCategory(data?._id)}
                            >
                              <i className="fa fa-pencil" />
                            </button>
                            <button
                              className="btn btn-xs sharp btn-danger"
                              onClick={() => handleDeleteSubCategory(data?._id)}
                            >
