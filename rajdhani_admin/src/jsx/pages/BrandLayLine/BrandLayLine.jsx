...
import { addBrandLayLineApi, deleteBrandLayLineApi, getBrandLayLineApi,
   GetEditBrandLayLineData, UpdateBrandLayLine, UpdateBrandLayLineStatus } from "../../../services/apis/BrandLayLine";
...
const BrandLayLine = () => {
  ...
  const [brandLayLineList, setBrandLayLineList] = useState([]);
  ...
  //   getSubCategoriesApi
  const fetchBrandLayLine = async (sortValue) => {
    // Set loading to true when the API call starts
    setLoading(true);
    try {
      const res = await getBrandLayLineApi(
        currentPage,
        sort,
        sortValue,
        searchInputValue
      );
      setBrandLayLineList(res?.data?.brandLayLines);
  ...
  const handleEditBrandLayLine = async (id) => {
    try {
      const res = await GetEditBrandLayLineData(id);
      if (res?.data?.success) {
        const data = res?.data?.brandLayLine;
  ...
  const handleDeleteBrandLayLine = (id) => {
  ...
  const handleDeleteSubmit = async () => {
    try {
      const res = await deleteBrandLayLineApi(deleteTableDataId);
      if (res.status === 200) {
  ...
  const handleStatusChange = async (partId, currentStatus) => {
    ...
    try {
      const res = await UpdateBrandLayLineStatus(partId, fdata);
      if (res.status === 200) {
        Toaster.success(res?.data?.message); // Display success message
        fetchBrandLayLine();
      } else {
  ...
  const handleUpdateSubmit = async () => {
    try {
      const res = await UpdateBrandLayLine(editPartId, formData);
      if (res.status === 200) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (isEdit) {
      ...
      setLoading(true);
      try {
        const res = await addBrandLayLineApi(formData);
        if (res.data?.success) {
          setLoading(false);
          Toaster.success(res?.data?.message);
  ...
  return (
    <>
      ...
      <PageTitle activeMenu={"Brand Lay Line"} motherMenu={"Home"} />
      <div className="row">
        {/* SECTION 1ST */}
      ...
      <Row>
        <Col lg={12}>
          <div className="card">
            <div className="card-header">
              <h4 className="card-title">Brand Lay Line List</h4>
              ...
              <div className="table-responsive">
                <div id="holidayList" className="dataTables_wrapper no-footer">
                  ...
                  <table
                    ...
                    <tbody>
                      {brandLayLineList?.brandLayLines?.map((data, ind) => (
                        <tr key={ind}>
                          <td>
                            {/* <strong>{ind + 1}</strong> */}
                            <strong>
                          ...
                          <td>
                            <button
                              ...
                              onClick={() => handleEditBrandLayLine(data?._id)}>
                              <i className="fa fa-pencil" />
                            </button>
                            <button
                              className="btn btn-xs sharp btn-danger"
                              onClick={() => handleDeleteBrandLayLine(data?._id)}>
                  ...
                  <div>
                    ...
                    <div className="d-sm-flex text-center justify-content-end align-items-center mt-3">
                      <div className="pagination-container">
                        <ReactPaginate
                          pageCount={Math.ceil(
                            brandLayLineList?.totalBrandLayLines / brandLayLineList?.rowsPerPage
                          )}
