...
import { addProductionProcessItemLogsApi, addProductionProcessLog, endHoseCuttingProcess, endPackingProcessApi, endSkivingProcessApi, getAllOperatorsListApi, getLineNumberOptionsApi, startHoseCuttingProcess, startPackingProcessApi, startSkivingProcessApi, updateProductionProcessItemsApi, updateSkivingStatusApi } from "../../../../services/apis/ProductionProcessApi";
...
const PackingCheckPoints = [
  "Length, Batch, and Quantity Cross Check",
  "Proper Labeling and Sticker Placement",
  "Damage-Free Final Packing Verification"
];
...
const Packing = () => {
  ...
  const handleStartStage = async (id) => {
    ...
    try {
      const res = await startPackingProcessApi(id);
      ...
      Toaster.error("Server error while starting packing process.");
    } finally {
      setLoading(false);
      ...
      if (isSuccess) {
        Swal.fire({
          ...
          text: 'Packing process has been successfully started!',
          timer: 1000,
          showConfirmButton: false,
          timerProgressBar: true
        });
  ...
  const handleUpdateProductionItems = async (id) => {
    ...
    const payload = {
      itemDetails: {
        stage: "packing", // dynamic if needed -> produciton process model schema fields key
        production_process_id: producitonProcessDetails?.data?._id,
    ...
    try {
      ...
      if (updateItems?.data?.success === true) {
        ...
        const logData = {
          ...
          sheet_no: producitonProcessDetails?.data?.sheet_no,
          created_by: "shubham",
          type: 'PROCESS_PACKING_QUANTITY_UPDATE',
  ...
  const finalizePackingStage = async (producitonProcessId) => {
    try {
      const res = await endPackingProcessApi(producitonProcessId);
      ...
      if (res?.data?.success) {
        ...
        if (reslog) {
          Swal.fire({
            icon: 'success',
            title: 'Packing process finalized successfully',
            showConfirmButton: false,
            timer: 1000
          });
        }
  ...
  const orderDetails = {
    ...
    footer: {
      ...
      quality_assurance: [
        // Packing
        "✅ Final Length, Batch, and Quantity Cross Checked.",
        "✅ Proper Labeling and Sticker Placement Verified.",
        "✅ Final Packing is Damage-Free and Clean.",
        "✅ Packing Material as per Standard Used."
  ...
  useEffect(() => {
    if (producitonProcessItems?.length > 0) {
      const formattedRows = producitonProcessItems?.map((item) => ({
        ...
        id: item?._id,
        quantity_accepted: '',
        quantity_rejected: item?.packingStage?.quantityRejected || '',
        last_updated_quantity: item?.packingStage?.lastUpdatedQuantity,
        available_cutting_quantity: item?.testingStage?.lastUpdatedQuantity,
        remark: item?.packingStage?.remark || '',
        line_number: item?.packingStage?.line_number || '',
        ...(item?.packingStage?.operatorName && {
          operator_name: item?.packingStage?.operatorName,
        }),
        ...(item?.packingStage?.operatorId && {
          operator_id: item?.packingStage?.operatorId,
        }),
        log: item?.log || '',
  ...
  return (
    <>
                ...
                <div className="card-header pb-0 px-0">
                  <h1 className="card-title" style={{
                    ...
                    <span style={{
                      ...
                      {producitonProcessDetails?.data?.packing?.start_time
                        ? `Starting Time and Date: ${moment(producitonProcessDetails?.data?.packing?.start_time).format("DD MMM YYYY | hh:mm A")}`
                        : ''}
                    </span>
                  </h1>
                  <div className="mb-3">
                    ...
                    <div class="d-flex flex-wrap gap-2 mt-2">
                      ...
                      <div class="p-2 text-dark fw-bold border rounded" style={{ backgroundColor: "#8F87F1" }} >
                        Total Quantity: {producitonProcessItemsAnalytics?.packingStageAnalytics?.totalSheetQuantity ? `${producitonProcessItemsAnalytics?.packingStageAnalytics?.totalSheetQuantity}` : 0}
                      </div>
                      <div class="p-2 text-dark fw-bold border rounded" style={{ backgroundColor: "#FFF287" }}>
                        Total Pending: {producitonProcessItemsAnalytics?.packingStageAnalytics?.totalSheetQuantity ? `${producitonProcessItemsAnalytics?.packingStageAnalytics?.totalPendingQuantity}/${producitonProcessItemsAnalytics?.packingStageAnalytics?.totalSheetQuantity}` : 0}
                      </div>
                      <div class="p-2 text-dark fw-bold border rounded" style={{ backgroundColor: "#F7A8C4" }} >
                        Total Rejected: {producitonProcessItemsAnalytics?.packingStageAnalytics?.totalSheetQuantity ? `${producitonProcessItemsAnalytics?.packingStageAnalytics?.totalRejectedQuantity}/${producitonProcessItemsAnalytics?.packingStageAnalytics?.totalSheetQuantity}` : 0}
                      </div>
                      <div class="p-2 text-dark fw-bold border rounded" style={{ backgroundColor: "#AEC8A4" }} >
                        Total Completed: {producitonProcessItemsAnalytics?.packingStageAnalytics?.totalSheetQuantity ? `${producitonProcessItemsAnalytics?.packingStageAnalytics?.totalCompletedQuantity}/${producitonProcessItemsAnalytics?.packingStageAnalytics?.totalSheetQuantity}` : 0}
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="card-body px-0">
                  <div className="table-responsive">
                    <div
                      ...
                      {(() => {
                        const stageStatus = producitonProcessDetails?.data?.packing?.status;
                        ...
                        if (stageStatus === "Not Started" || stageStatus == null) {
                          ...
                          if (stageStatus === "Not Started" && producitonProcessDetails?.data?.packing?.status === "Completed") {
                            return <StageCompletedCard
                              supervisorName={producitonProcessDetails?.data?.finalized_by || 'Supervisor'}
                              completedAt={producitonProcessDetails?.data?.packing?.end_time}
                            />
                          } else {
                            return (
      ...
      <QualityAssuranceModal
        ...
        stage="Packing"
        checkPointsQA={PackingCheckPoints}
        onFinalize={() => finalizePackingStage(producitonProcessDetails?.data?._id)}
      />
    </>
  );
};
