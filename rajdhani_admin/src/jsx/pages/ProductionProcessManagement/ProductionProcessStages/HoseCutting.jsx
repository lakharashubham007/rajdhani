...
import { addProductionProcessItemLogsApi, addProductionProcessLog, endHoseCuttingProcess, getAllOperatorsListApi, getLineNumberOptionsApi, startHoseCuttingProcess, updateProductionProcessItemsApi } from "../../../../services/apis/ProductionProcessApi";
...
const HoseCutting = () => {
    ...
    const validateField = (field, value, row) => {
        ...
        if (field === "quantity_accepted") {
            const maxAcceptable = totalQty - lastUpdated - rejectedQty;
        ...
        if (field === "quantity_rejected") {
            const maxRejectable = totalQty - lastUpdated - acceptedQty;
    ...
    const handleStartStage = async (id) => {
        ...
        try {
            const res = await startHoseCuttingProcess(id);
            ...
            if (res?.data?.success) {
                ...
                const logData = {
                    ...
                    created_by: "system",
                    type: 'PROCESS_HOSE_CUTTING_START'
                };
            ...
            if (isSuccess) {
                Swal.fire({
                    ...
                    text: 'Cutting process has been successfully started!',
                    timer: 2000,
                    showConfirmButton: false,
                    timerProgressBar: true
                });
    ...
    const handleUpdateProductionItems = async (id) => {
        ...
        rows.forEach((item) => {
            ...
            const isWithinAcceptedLimit = hasAcceptedQty && totalUsedQty <= totalQty;

            const isWithinRejectedOnlyLimit =
                !hasAcceptedQty && hasRejectedQty && quantityRejected + lastUpdated <= totalQty;
        ...
        const payload = {
            itemDetails: {
                stage: "hose_cutting", // dynamic if needed
                production_process_id: producitonProcessDetails?.data?._id,
        ...
        try {
            ...
            if (updateItems?.data?.success === true) {
                ...
                const logData = {
                    ...
                    sheet_no: producitonProcessDetails?.data?.sheet_no,
                    created_by: "shubham",
                    type: 'PROCESS_HOSE_CUTTING_QUANTITY_UPDATE',
    ...
    const finalizeHoseCuttingStage = async (producitonProcessId) => {
        try {
            const res = await endHoseCuttingProcess(producitonProcessId);
            ...
            if (res?.data?.success) {
                ...
                if (reslog) {
                    Swal.fire({
                        icon: 'success',
                        title: 'Cutting process finalized successfully',
                        showConfirmButton: false,
                        timer: 1000
                    });
                }
    ...
    // const handleFinalSubmission = async (producitonProcessId) => {
    ...
    //         Swal.fire({
    //             title: 'Incomplete Stage',
    //             text: 'Stage cannot be finalized. Some item quantities are still pending.',
    //             icon: 'info',
    //             confirmButtonText: 'OK',
    //             timer: 5000,
    //             timerProgressBar: true,
    ...
    //     }).then((result) => {
    //         if (result.isConfirmed) {
    //             setShowQAModal(true); // Show QA modal

    //         }
    //     });
    //     try {
    //         const res = await endHoseCuttingProcess(producitonProcessId); // API should support search
    //         console.log("res", res);
    //         if (res.data.success) {
    //              //upate production process imediatly
    //             dispatch(getProductionProcessDetailsBySheetIDRequest(producitonProcessDetails?.data?.production_sheet_id));

    //               // ✅ Create log entry with new info
    //             const logData = {
    //                 production_process_id: producitonProcessDetails?.data?._id,
    //                 sheet_no: producitonProcessDetails?.data?.sheet_no,
    //                 created_by: "system",
    ...
    //             if (reslog) {
    //                 Toaster.success("Cutting process Finished successfully.");
    //             }
    ...
    useEffect(() => {
        ...
        if (producitonProcessItems?.length > 0) {
            const formattedRows = producitonProcessItems?.map((item) => ({
                ...
                id: item?._id,
                quantity_accepted: '',
                quantity_rejected: item?.hoseCuttingStage?.quantityRejected || '',
                last_updated_quantity: item?.hoseCuttingStage?.lastUpdatedQuantity,
                remark: item?.remark || '',
                line_number: item?.hoseCuttingStage?.line_number || '',
                ...(item?.hoseCuttingStage?.operatorName && {
                    operator_name: item?.hoseCuttingStage?.operatorName,
                }),
                ...(item?.hoseCuttingStage?.operatorId && {
                    operator_id: item?.hoseCuttingStage?.operatorId,
                }),
                log: item?.log || '',
    ...
    return (
        <>
                                    ...
                                    <div className="mb-3">
                                        ...
                                        <div class="d-flex flex-wrap gap-2 mt-2">
                                            ...
                                            <div class="p-2 text-dark fw-bold border rounded" style={{ backgroundColor: "#8F87F1" }} >
                                                Total Quantity: {producitonProcessItemsAnalytics?.hoseCuttingStageAnalytics?.totalSheetQuantity ? `${producitonProcessItemsAnalytics?.hoseCuttingStageAnalytics?.totalSheetQuantity}` : 0}
                                            </div>
                                            <div class="p-2 text-dark fw-bold border rounded" style={{ backgroundColor: "#FFF287" }}>
                                                Total Pending: {producitonProcessItemsAnalytics?.hoseCuttingStageAnalytics?.totalSheetQuantity ? `${producitonProcessItemsAnalytics?.hoseCuttingStageAnalytics?.totalPendingQuantity}/${producitonProcessItemsAnalytics?.hoseCuttingStageAnalytics?.totalSheetQuantity}` : 0}
                                            </div>
                                            <div class="p-2 text-dark fw-bold border rounded" style={{ backgroundColor: "#F7A8C4" }} >
                                                Total Rejected: {producitonProcessItemsAnalytics?.hoseCuttingStageAnalytics?.totalSheetQuantity ? `${producitonProcessItemsAnalytics?.hoseCuttingStageAnalytics?.totalRejectedQuantity}/${producitonProcessItemsAnalytics?.hoseCuttingStageAnalytics?.totalSheetQuantity}` : 0}
                                            </div>
                                            <div class="p-2 text-dark fw-bold border rounded" style={{ backgroundColor: "#AEC8A4" }} >
                                                Total Completed: {producitonProcessItemsAnalytics?.hoseCuttingStageAnalytics?.totalSheetQuantity ? `${producitonProcessItemsAnalytics?.hoseCuttingStageAnalytics?.totalCompletedQuantity}/${producitonProcessItemsAnalytics?.hoseCuttingStageAnalytics?.totalSheetQuantity}` : 0}
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                        ...
                                        <div
                                            ...
                                            {(() => {
                                                ...
                                                if (stageStatus === "Not Started" || stageStatus == null) {
                                                    ...
                                                    if (stageStatus === "Not Started" && producitonProcessDetails?.data?.hose_cutting?.status === "Completed") {
                                                        return <StageCompletedCard
                                                            supervisorName={producitonProcessDetails?.data?.finalized_by || 'Supervisor'}
                                                            completedAt={producitonProcessDetails?.data?.hose_cutting?.end_time}
                                                        />
                                                    } else {
                                                        return (
            ...
            <QualityAssuranceModal
                ...
                stage="Hose Cutting"
                checkPointsQA={HoseCuttingCheckPoints}
                onFinalize={() => finalizeHoseCuttingStage(producitonProcessDetails?.data?._id)}
            />
        </>
    );
};

export default HoseCutting;
