...
import { addProductionProcessItemLogsApi, addProductionProcessLog, endHoseCuttingProcess, endPreAssemblyProcessApi, getAllOperatorsListApi, getLineNumberOptionsApi, startHoseCuttingProcess, startPreAssemblyProcessApi, updateProductionProcessItemsApi } from "../../../../services/apis/ProductionProcessApi";
...
const PreAssemblyCheckPoints = [
  "Hose Size & Type & Spring Cross Check",
  "Insertion Depth Marking Cross Check",
  "Hose Length Cross Check",
  "Check End-1 & End-2",
  "Orientation Angle Marking"
];
...
const theadData = [
  ...
  { heading: "Available Quantity ", sortingVale: "available_quantity" },
  { heading: "Skiving Stage Status ", sortingVale: "skving_stage_status" },
...
const PreAssembly = () => {
  ...
  //Validaiton for Quaniity in preassembly stage is different from hose cutting stage = here avalable quanity also checked.
  const validateField = (field, value, row) => {
    ...
    const availableCuttingQty = Number(row?.available_quantity || 0);
  ...
  const handleStartStage = async (id) => {
    ...
    try {
      const res = await startPreAssemblyProcessApi(id);
      ...
      if (res?.data?.success) {
        ...
        const logData = {
          ...
          created_by: "system",
          type: 'PROCESS_PRE_ASSEMBLY_START'
        };
        const reslog = await addProductionProcessLog(logData);

        if (reslog?.data?.success) {
          // console.log("Cutting Process Started Successfully");
      ...
      Toaster.error("Server error while starting cutting process.");
    } finally {
      setLoading(false);
      ...
      if (isSuccess) {
        Swal.fire({
          icon: 'success',
          title: 'Pre-Assembly Started',
          text: 'Pre-Assembly process has been successfully started!',
          timer: 2000,
          showConfirmButton: false,
          timerProgressBar: true
        });
  ...
  const handleUpdateProductionItems = async (id) => {
    ...
    rows.forEach((item) => {
      ...
      const avalQty = Number(item?.available_quantity) || 0;
    ...
    const payload = {
      itemDetails: {
        stage: "pre_assembly", // dynamic if needed // productionProcessItemsService -> STAGE_MAP  field
        production_process_id: producitonProcessDetails?.data?._id,
    ...
    try {
      ...
      // console.log("updateItems?.data -=-=-=-=-=-=-=----------->", updateItems)
      if (updateItems?.data?.success === true) {
        ...
        const logData = {
          ...
          sheet_no: producitonProcessDetails?.data?.sheet_no,
          created_by: "shubham",
          type: 'PROCESS_PRE_ASSEMBLY_QUANTITY_UPDATE',
        ...
        if (productionProcessLogDetails?.data?.success === true && logId) {
          ...
          const itemLogsPayload = validItems.map(item => ({
            log_id: logId,
            item_id: item.id,
            stage: "Pre-assembly", // from mongodb productionProcessItemLog -> Model -> Stage
            ...
            remarks: item.remark || "",
            operatorNames: item?.operator_name,
      ...
      if (updateItems?.data?.success === true) {
        ...
        // Toaster.success(updateItems?.data?.message || "Items updated!");
      }
      ...
      // setLoading(false);
      Toaster.error("Failed to update items. Please try again.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const finalizePreAssemblyStage = async (producitonProcessId) => {
    try {
      const res = await endPreAssemblyProcessApi(producitonProcessId);
      ...
      if (res?.data?.success) {
        ...
        if (reslog) {
          Swal.fire({
            icon: 'success',
            title: 'Pre-Assembly process finalized successfully',
            showConfirmButton: false,
            timer: 1000
          });
        }
  ...
  const handleFinalSubmission = (producitonProcessId) => {
    const totalSheetQuantity = producitonProcessItemsAnalytics?.hoseCuttingStageAnalytics?.totalSheetQuantity || 0;
    const totalCompletedQuantity = producitonProcessItemsAnalytics?.hoseCuttingStageAnalytics?.totalCompletedQuantity || 0;
  ...
  const orderDetails = {
    ...
    footer: {
      ...
      quality_assurance: [
        "✅ Hose Size, Type & Spring Verified.",
        "✅ Insertion Depth Marking Cross Checked.",
        "✅ Hose Length Reconfirmed.",
        "✅ End-1 & End-2 Parts Matched and Installed Properly.",
        "✅ Orientation Angle Marking Verified.",
      ]
    },
  };
  ...
  useEffect(() => {
    ...
    // }
  }, [producitonProcessDetails, quantityUpdated, sort, currentPage])
  ...
  useEffect(() => {
    ...
    if (producitonProcessItems?.length > 0) {
      const formattedRows = producitonProcessItems?.map((item) => ({
        ...
        id: item?._id,
        quantity_accepted: '',
        quantity_rejected: item?.preAssemblyStage?.quantityRejected || '',
        last_updated_quantity: item?.preAssemblyStage?.lastUpdatedQuantity,
        available_quantity: item?.skivingStage?.lastUpdatedQuantity,
        isSkiving: item?.skivingStage?.isSkiving,
        remark: item?.remark || '',
        line_number: item?.preAssemblyStage?.line_number || '',
        ...(item?.preAssemblyStage?.operatorName && {
          operator_name: item?.preAssemblyStage?.operatorName,
        }),
        ...(item?.preAssemblyStage?.operatorId && {
          operator_id: item?.preAssemblyStage?.operatorId,
        }),
        log: item?.log || '',
  ...
  return (
    <>
                ...
                <div className="card-header pb-0 px-0">
                  <h1 className="card-title" style={{
                    ...
                    <span style={{
                      ...
                      {producitonProcessDetails?.data?.hose_cutting?.start_time
                        ? `Starting Time and Date: ${moment(producitonProcessDetails?.data?.hose_cutting.start_time).format("DD MMM YYYY | hh:mm A")}`
                        : ''}
                    </span>
                  </h1>
                  <div className="mb-3">
                    ...
                    <div class="d-flex flex-wrap gap-2 mt-2">
                      ...
                      <div class="p-2 text-dark fw-bold border rounded" style={{ backgroundColor: "#8F87F1" }} >
                        Total Quantity: {producitonProcessItemsAnalytics?.preAssemblyStageAnalytics?.totalSheetQuantity ? `${producitonProcessItemsAnalytics?.preAssemblyStageAnalytics?.totalSheetQuantity}` : 0}
                      </div>
                      <div class="p-2 text-dark fw-bold border rounded" style={{ backgroundColor: "#FFF287" }}>
                        Total Pending: {producitonProcessItemsAnalytics?.preAssemblyStageAnalytics?.totalSheetQuantity ? `${producitonProcessItemsAnalytics?.preAssemblyStageAnalytics?.totalPendingQuantity}/${producitonProcessItemsAnalytics?.preAssemblyStageAnalytics?.totalSheetQuantity}` : 0}
                      </div>
                      <div class="p-2 text-dark fw-bold border rounded" style={{ backgroundColor: "#F7A8C4" }} >
                        Total Rejected: {producitonProcessItemsAnalytics?.preAssemblyStageAnalytics?.totalSheetQuantity ? `${producitonProcessItemsAnalytics?.preAssemblyStageAnalytics?.totalRejectedQuantity}/${producitonProcessItemsAnalytics?.preAssemblyStageAnalytics?.totalSheetQuantity}` : 0}
                      </div>
                      <div class="p-2 text-dark fw-bold border rounded" style={{ backgroundColor: "#AEC8A4" }} >
                        Total Completed: {producitonProcessItemsAnalytics?.preAssemblyStageAnalytics?.totalSheetQuantity ? `${producitonProcessItemsAnalytics?.preAssemblyStageAnalytics?.totalCompletedQuantity}/${producitonProcessItemsAnalytics?.preAssemblyStageAnalytics?.totalSheetQuantity}` : 0}
                      </div>
                    </div>
                  </div>
                </div>
                {/* Production Process Items Table  */}
                <div className="card-body px-0">
                  <div className="table-responsive">
                    <div
                      ...
                      {(() => {
                        const stageStatus = producitonProcessDetails?.data?.pre_assembly?.status;
                        ...
                        if (stageStatus === "Not Started" || stageStatus == null) {
                          ...
                          if (stageStatus === "Not Started" && producitonProcessDetails?.data?.pre_assembly?.status === "Completed") {
                            return <StageCompletedCard
                              supervisorName={producitonProcessDetails?.data?.finalized_by || 'Supervisor'}
                              completedAt={producitonProcessDetails?.data?.pre_assembly?.end_time}
                            />
                          } else {
                            return (
                                  ...
                                  {rows?.map((row, index) => (
                                    <tr key={row.id}
                                      ...
                                      <td style={{ minWidth: '120px' }}>
                                        <div className="progress-indicator">
                                          <div className="progress-label">
                                            {row?.available_quantity}/{row?.sheet_total_quantity}
                                          </div>
                                          <div className="progress-bar-container">
                                            <div
                                              className="progress-bar-fill"
                                              style={{
                                                width: `${(Number(row?.available_quantity || 0) /
                                      ...
                                      <td style={{ minWidth: '50px' }}>
                                        ...
                                        <span
                                          className={`skiving-badge ${row?.isSkiving ? 'skived' : 'non-skived'}`}
                                        >
                                          {row?.isSkiving ? 'Skived' : 'Non-Skived'}
                                        </span>
      ...
      <QualityAssuranceModal
        ...
        stage="Pre Assembly"
        checkPointsQA={PreAssemblyCheckPoints}
        onFinalize={() => finalizePreAssemblyStage(producitonProcessDetails?.data?._id)}
      />
    </>
  );
};
