...
import { addProductionProcessItemLogsApi, addProductionProcessLog, endHoseCuttingProcess, endSkivingProcessApi, endTestingProcessApi, getAllOperatorsListApi, getLineNumberOptionsApi, startHoseCuttingProcess, startSkivingProcessApi, startTestingProcessApi, updateProductionProcessItemsApi, updateSkivingStatusApi } from "../../../../services/apis/ProductionProcessApi";
...
const TestingCheckPoints = [
  ...
  "Insertion Depth Marking After Crimping",
  "Displacement Angle Marking Cross Check",
  "Crimping Quality Marking Cross Check",
  "Leak Testing Pressure",
  '"Teak Tested Ok" Sticker'
];
const theadData = [
...
const Testing = () => {
  ...
  const handleStartStage = async (id) => {
    ...
    try {
      const res = await startTestingProcessApi(id);
      ...
      Toaster.error("Server error while starting testing process.");
    } finally {
      setLoading(false);
      ...
      if (isSuccess) {
        Swal.fire({
          ...
          text: 'Testing process has been successfully started!',
          timer: 1000,
          showConfirmButton: false,
          timerProgressBar: true
        });
  ...
  const handleUpdateProductionItems = async (id) => {
    ...
    const payload = {
      itemDetails: {
        stage: "testing", // dynamic if needed -> produciton process model schema fields key
        production_process_id: producitonProcessDetails?.data?._id,
    ...
    try {
      ...
      if (updateItems?.data?.success === true) {
        ...
        const logData = {
          ...
          sheet_no: producitonProcessDetails?.data?.sheet_no,
          created_by: "shubham",
          type: 'PROCESS_TESTING_QUANTITY_UPDATE',
  ...
  const finalizeTestingStage = async (producitonProcessId) => {
    try {
      const res = await endTestingProcessApi(producitonProcessId);
      ...
      if (res?.data?.success) {
        ...
        if (reslog) {
          Swal.fire({
            icon: 'success',
            title: 'Testing process finalized successfully',
            showConfirmButton: false,
            timer: 1000
          });
        }
  ...
  const orderDetails = {
    ...
    footer: {
      ...
      quality_assurance: [
        // Testing
        "✅ Hose Size, Type & Spring Cross Checked.",
        "✅ Insertion Depth Marking After Crimping Verified.",
        "✅ Displacement Angle Marking Confirmed.",
        "✅ Crimping Quality Verified During Leak Test.",
        "✅ Leak Testing Pressure Within Spec.",
        "✅ 'Tested OK' Sticker Applied.",
      ]
    },
  };
  ...
  useEffect(() => {
    ...
    if (producitonProcessItems?.length > 0) {
      const formattedRows = producitonProcessItems?.map((item) => ({
        ...
        id: item?._id,
        quantity_accepted: '',
        quantity_rejected: item?.testingStage?.quantityRejected || '',
        last_updated_quantity: item?.testingStage?.lastUpdatedQuantity,
        available_cutting_quantity: item?.crimpingStage?.lastUpdatedQuantity,
        // isSkiving: item?.skivingStage?.isSkiving,
        remark: item?.testingStage?.remark || '',
        line_number: item?.testingStage?.line_number || '',
        ...(item?.testingStage?.operatorName && {
          operator_name: item?.testingStage?.operatorName,
        }),
        ...(item?.testingStage?.operatorId && {
          operator_id: item?.testingStage?.operatorId,
        }),
        log: item?.log || '',
  ...
  return (
    <>
                ...
                <div className="card-header pb-0 px-0">
                  <h1 className="card-title" style={{
                    ...
                    <span style={{
                      ...
                      {producitonProcessDetails?.data?.testing?.start_time
                        ? `Starting Time and Date: ${moment(producitonProcessDetails?.data?.testing?.start_time).format("DD MMM YYYY | hh:mm A")}`
                        : ''}
                    </span>
                  </h1>
                  <div className="mb-3">
                    ...
                    <div class="d-flex flex-wrap gap-2 mt-2">
                      ...
                      <div class="p-2 text-dark fw-bold border rounded" style={{ backgroundColor: "#8F87F1" }} >
                        Total Quantity: {producitonProcessItemsAnalytics?.testingStageAnalytics?.totalSheetQuantity ? `${producitonProcessItemsAnalytics?.testingStageAnalytics?.totalSheetQuantity}` : 0}
                      </div>
                      <div class="p-2 text-dark fw-bold border rounded" style={{ backgroundColor: "#FFF287" }}>
                        Total Pending: {producitonProcessItemsAnalytics?.testingStageAnalytics?.totalSheetQuantity ? `${producitonProcessItemsAnalytics?.testingStageAnalytics?.totalPendingQuantity}/${producitonProcessItemsAnalytics?.testingStageAnalytics?.totalSheetQuantity}` : 0}
                      </div>
                      <div class="p-2 text-dark fw-bold border rounded" style={{ backgroundColor: "#F7A8C4" }} >
                        Total Rejected: {producitonProcessItemsAnalytics?.testingStageAnalytics?.totalSheetQuantity ? `${producitonProcessItemsAnalytics?.testingStageAnalytics?.totalRejectedQuantity}/${producitonProcessItemsAnalytics?.testingStageAnalytics?.totalSheetQuantity}` : 0}
                      </div>
                      <div class="p-2 text-dark fw-bold border rounded" style={{ backgroundColor: "#AEC8A4" }} >
                        Total Completed: {producitonProcessItemsAnalytics?.testingStageAnalytics?.totalSheetQuantity ? `${producitonProcessItemsAnalytics?.testingStageAnalytics?.totalCompletedQuantity}/${producitonProcessItemsAnalytics?.testingStageAnalytics?.totalSheetQuantity}` : 0}
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="card-body px-0">
                  <div className="table-responsive">
                    <div
                      ...
                      {(() => {
                        const stageStatus = producitonProcessDetails?.data?.testing?.status;
                        ...
                        if (stageStatus === "Not Started" || stageStatus == null) {
                          ...
                          if (stageStatus === "Not Started" && producitonProcessDetails?.data?.testing?.status === "Completed") {
                            return <StageCompletedCard
                              supervisorName={producitonProcessDetails?.data?.finalized_by || 'Supervisor'}
                              completedAt={producitonProcessDetails?.data?.testing?.end_time}
                            />
                          } else {
                            return (
      ...
      <QualityAssuranceModal
        ...
        stage="Testing"
        checkPointsQA={TestingCheckPoints}
        onFinalize={() => finalizeTestingStage(producitonProcessDetails?.data?._id)}
      />
    </>
  );
};
