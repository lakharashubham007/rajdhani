...
import { addProductionProcessItemLogsApi, addProductionProcessLog, endHoseCuttingProcess, endSkivingProcessApi, getAllOperatorsListApi, getLineNumberOptionsApi, startHoseCuttingProcess, startSkivingProcessApi, updateProductionProcessItemsApi, updateSkivingStatusApi } from "../../../../services/apis/ProductionProcessApi";
...
const SkivingCheckPoints = [
  "Hose Size & Type Cross Check",
...
const theadData = [
  ...
  { heading: "Skiving", sortingVale: "skiving" },
...
const Skiving = () => {
  ...
  const handleStartStage = async (id) => {
    ...
    try {
      const res = await startSkivingProcessApi(id);
      ...
      if (isSuccess) {
        Swal.fire({
          icon: 'success',
          title: 'Skiving Started',
          text: 'Skiving process has been successfully started!',
          timer: 2000,
          showConfirmButton: false,
          timerProgressBar: true
        });
  ...
  const handleUpdateProductionItems = async (id) => {
    ...
    const payload = {
      itemDetails: {
        stage: "skiving", // dynamic if needed -> produciton process model schema fields key
        production_process_id: producitonProcessDetails?.data?._id,
    ...
    try {
      ...
      if (updateItems?.data?.success === true) {
        ...
        const logData = {
          ...
          sheet_no: producitonProcessDetails?.data?.sheet_no,
          created_by: "shubham",
          type: 'PROCESS_SKIVING_QUANTITY_UPDATE',
  ...
  const finalizeSkivingStage = async (producitonProcessId) => {
    try {
      const res = await endSkivingProcessApi(producitonProcessId);
      ...
      if (res?.data?.success) {
        ...
        if (reslog) {
          Swal.fire({
            icon: 'success',
            title: 'Skiving process finalized successfully',
            showConfirmButton: false,
            timer: 1000
          });
        }
  ...
  const handleFinalSubmission = (producitonProcessId) => {
    const totalSheetQuantity = producitonProcessItemsAnalytics?.skivingStageAnalytics?.totalSheetQuantity || 0;
    const totalCompletedQuantity = producitonProcessItemsAnalytics?.skivingStageAnalytics?.totalCompletedQuantity || 0;
  ...
  const handleSkivingStatus = async (itemId, isSkiving) => {
    try {
      const payload = {
        item_id: itemId,
        stage: 'skivingStage',
      ...
      const response = await updateSkivingStatusApi(payload);

      if (response?.data?.success) {
        setIsSkivingStatusUpdated(true);
      } else {
        Swal.fire({
          icon: 'error',
          title: 'Update Failed',
          text: 'Could not update skiving status. Please try again.',
        });
      }
    } catch (error) {
      console.error("Skiving update error:", error);
      Swal.fire({
        icon: 'error',
        title: 'Error',
        text: 'Something went wrong while updating skiving status.',
      });
    }
  };
  ...
  useEffect(() => {
    ...
    if (producitonProcessItems?.length > 0) {
      const formattedRows = producitonProcessItems?.map((item) => ({
        ...
        id: item?._id,
        quantity_accepted: '',
        quantity_rejected: item?.skivingStage?.quantityRejected || '',
        last_updated_quantity: item?.skivingStage?.lastUpdatedQuantity,
        available_cutting_quantity: item?.hoseCuttingStage?.lastUpdatedQuantity,
        isSkiving: item?.skivingStage?.isSkiving,
        remark: item?.skivingStage?.remark || '',
        line_number: item?.skivingStage?.line_number || '',
        ...(item?.skivingStage?.operatorName && {
          operator_name: item?.skivingStage?.operatorName,
        }),
        ...(item?.skivingStage?.operatorId && {
          operator_id: item?.skivingStage?.operatorId,
        }),
        log: item?.log || '',
  ...
  return (
    <>
                  ...
                  <h1 className="card-title" style={{
                    ...
                    <span style={{
                      ...
                      {producitonProcessDetails?.data?.hose_cutting?.start_time
                        ? `Starting Time and Date: ${moment(producitonProcessDetails?.data?.skiving.start_time).format("DD MMM YYYY | hh:mm A")}`
                        : ''}
                    </span>
                  </h1>
                  <div className="mb-3">
                    ...
                    <div class="d-flex flex-wrap gap-2 mt-2">
                      ...
                      <div class="p-2 text-dark fw-bold border rounded" style={{ backgroundColor: "#8F87F1" }} >
                        Total Quantity: {producitonProcessItemsAnalytics?.skivingStageAnalytics?.totalSheetQuantity ? `${producitonProcessItemsAnalytics?.skivingStageAnalytics?.totalSheetQuantity}` : 0}
                      </div>
                      <div class="p-2 text-dark fw-bold border rounded" style={{ backgroundColor: "#FFF287" }}>
                        Total Pending: {producitonProcessItemsAnalytics?.skivingStageAnalytics?.totalSheetQuantity ? `${producitonProcessItemsAnalytics?.skivingStageAnalytics?.totalPendingQuantity}/${producitonProcessItemsAnalytics?.skivingStageAnalytics?.totalSheetQuantity}` : 0}
                      </div>
                      <div class="p-2 text-dark fw-bold border rounded" style={{ backgroundColor: "#F7A8C4" }} >
                        Total Rejected: {producitonProcessItemsAnalytics?.skivingStageAnalytics?.totalSheetQuantity ? `${producitonProcessItemsAnalytics?.skivingStageAnalytics?.totalRejectedQuantity}/${producitonProcessItemsAnalytics?.skivingStageAnalytics?.totalSheetQuantity}` : 0}
                      </div>
                      <div class="p-2 text-dark fw-bold border rounded" style={{ backgroundColor: "#AEC8A4" }} >
                        Total Completed: {producitonProcessItemsAnalytics?.skivingStageAnalytics?.totalSheetQuantity ? `${producitonProcessItemsAnalytics?.skivingStageAnalytics?.totalCompletedQuantity}/${producitonProcessItemsAnalytics?.skivingStageAnalytics?.totalSheetQuantity}` : 0}
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="card-body px-0">
                  <div className="table-responsive">
                    <div
                      ...
                      {(() => {
                        const stageStatus = producitonProcessDetails?.data?.skiving?.status;
                        ...
                        if (stageStatus === "Not Started" || stageStatus == null) {
                          ...
                          if (stageStatus === "Not Started" && producitonProcessDetails?.data?.skiving?.status === "Completed") {
                            return <StageCompletedCard
                              supervisorName={producitonProcessDetails?.data?.finalized_by || 'Supervisor'}
                              completedAt={producitonProcessDetails?.data?.skiving?.end_time}
                            />
                          } else {
                            return (
                                ...
                                <tbody >
                                  {rows?.map((row, index) => (
                                    ...
                                    >
                                      ...
                                      {/* Skiving Applicable or not */}
                                      <td>
                                        <Select
                                          options={[
                                            { label: 'Applicable', value: true },
                                            { label: 'Non Applicable', value: false },
                                          ]}
                                          value={
                                            row?.isSkiving === true
                                              ? { label: 'Applicable', value: true }
                                          ...
                                          onChange={async (selectedOption) => {
                                            const isSkiving = selectedOption?.value ?? '';
                                            handleChangeRow(index, 'isSkiving', isSkiving);

                                            if (selectedOption) {
                                              const itemId = row?.id || row?._id;

                                              // ✅ Start loader
                                              setRowLoading((prev) => ({ ...prev, [itemId]: true }));

                                              try {
                                                await handleSkivingStatus(itemId, isSkiving);
                                              } finally {
                                                // ✅ Stop loader
                                                setRowLoading((prev) => ({ ...prev, [itemId]: false }));
                                              }
                                            }
                                          }}
                                          ...
                                          styles={{
                                            ...
                                            indicatorsContainer: (base) => ({ ...base, height: '28px', paddingTop: '2px' }),
                                            dropdownIndicator: (base) => ({ ...base, padding: '2px', marginTop: '2px' }),
                                            valueContainer: (base) => ({
                                              height: '100%',
                                              marginLeft: '5px',
                                              display: 'flex',
                                            ...
                                            indicatorSeparator: (base) => ({ ...base, height: '28px', margin: '0 4px' }),
      ...
      <QualityAssuranceModal
        ...
        stage="Skiving"
        checkPointsQA={SkivingCheckPoints}
        onFinalize={() => finalizeSkivingStage(producitonProcessDetails?.data?._id)}
      />
    </>
  );
};

export default Skiving;
