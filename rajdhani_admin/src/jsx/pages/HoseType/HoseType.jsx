...
import { addHoseTypeApi, deleteHoseTypeApi, GetEditHoseTypeData, 
  getHoseTypeApi, UpdateHoseType, UpdateHoseTypeStatus } from "../../../services/apis/HoseType";
...
const HoseType = () => {
  ...
  const [hoseTypeList, setHoseTypeList] = useState([]);
  ...
  //   getSubCategoriesApi
  const fetchHoseType = async (sortValue) => {
    // Set loading to true when the API call starts
    setLoading(true);
    try {
      const res = await getHoseTypeApi(
        currentPage,
        sort,
        sortValue,
        searchInputValue
      );
      setHoseTypeList(res?.data?.hoseTypes);
  ...
  const handleEditHoseType = async (id) => {
    try {
      const res = await GetEditHoseTypeData(id);
      if (res?.data?.success) {
        const data = res?.data?.hoseType;
  ...
  const handleDeleteSubmit = async () => {
    try {
      const res = await deleteHoseTypeApi(deleteTableDataId);
      if (res.status === 200) {
  ...
  const handleStatusChange = async (partId, currentStatus) => {
    ...
    try {
      const res = await UpdateHoseTypeStatus(partId, fdata);
      if (res.status === 200) {
        Toaster.success(res?.data?.message); // Display success message
        fetchHoseType();
      } else {
  ...
  const handleUpdateSubmit = async () => {
    try {
      const res = await UpdateHoseType(editPartId, formData);
      if (res.status === 200) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (isEdit) {
      ...
      setLoading(true);
      try {
        const res = await addHoseTypeApi(formData);
        if(res?.data?.success) {
          setLoading(false);
          Toaster.success(res?.data?.message);
  ...
  return (
    <>
      ...
      <PageTitle activeMenu={"Hose Type"} motherMenu={"Home"} />
      <div className="row">
        {/* SECTION 1ST */}
      ...
      <Row>
        <Col lg={12}>
          <div className="card">
            <div className="card-header">
              <h4 className="card-title">Hose Type List</h4>
              ...
              <div className="table-responsive">
                <div id="holidayList" className="dataTables_wrapper no-footer">
                  ...
                  <table
                    ...
                    <tbody>
                      {hoseTypeList?.hoseTypes?.map((data, ind) => (
                        <tr key={ind}>
                          <td>
                            {/* <strong>{ind + 1}</strong> */}
                            <strong>
                          ...
                          <td>
                            <button
                              ...
                              onClick={() => handleEditHoseType(data?._id)}>
                  ...
                  <div>
                    ...
                    <div className="d-sm-flex text-center justify-content-end align-items-center mt-3">
                      <div className="pagination-container">
                        <ReactPaginate
                          pageCount={Math.ceil(
                            hoseTypeList?.totalHoseTypes / hoseTypeList?.rowsPerPage
                          )}
