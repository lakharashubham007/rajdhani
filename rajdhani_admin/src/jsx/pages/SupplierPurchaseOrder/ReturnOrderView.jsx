...
import { GetReturnOrderViewData } from "../../../services/apis/purchaseOrderBillApi";
...
const ReturnOrderView = () => {
  ...
  const fetchPurchaseOrderViewData = async () => {
    try {
      const res = await GetReturnOrderViewData(params);
      const data = res?.data;
      // console.log("data is here",data.data);
      setPurchaseOrderProdcutList(data?.data?.return_order_items)
      setPurchaseOrderData(data?.data?.po_details);
    } catch (err) {
