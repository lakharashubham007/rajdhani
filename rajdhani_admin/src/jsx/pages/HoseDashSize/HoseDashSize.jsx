...
import { addHoseDashSizeApi, deleteHoseDashSizeApi, GetEditHoseDashSizeData,
         getHoseDashSizeApi, UpdateHoseDashSize, UpdateHoseDashSizeStatus } from "../../../services/apis/HoseDashSize";
...
const theadData = [
  ...
  { heading: "Dash Code", sortingVale: "dash_code" },
...
const HoseDashSize = () => {
  ...
  const [hoseDashSizeList, setHoseDashSizeList] = useState([]);
  ...
  //   getSubCategoriesApi
  const fetchHoseDashSize = async (sortValue) => {
    // Set loading to true when the API call starts
    setLoading(true);
    try {
      const res = await getHoseDashSizeApi(
        currentPage,
        sort,
        sortValue,
        searchInputValue
      );
      setHoseDashSizeList(res?.data?.hoseDashSizes);
  ...
  const handleEditHoseDashSize = async (id) => {
    try {
      const res = await GetEditHoseDashSizeData(id);
      if (res?.data?.success) {
        const data = res?.data?.hoseDashSize;
  ...
  const handleDeleteSubmit = async () => {
    try {
      const res = await deleteHoseDashSizeApi(deleteTableDataId);
      if (res.status === 200) {
  ...
  const handleStatusChange = async (partId, currentStatus) => {
    ...
    try {
      const res = await UpdateHoseDashSizeStatus(partId, fdata);
      if (res.status === 200) {
        Toaster.success(res?.data?.message); // Display success message
        fetchHoseDashSize();
      } else {
  ...
  const handleUpdateSubmit = async () => {
    ...
    try {
      const res = await UpdateHoseDashSize(editPartId, formD);
      if (res.status === 200) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (isEdit) {
      ...
      try {
        const res = await addHoseDashSizeApi(formD);
        if(res?.data?.success) {
          setLoading(false);
          Toaster.success(res?.data?.message);
  ...
  return (
    <>
      ...
      <PageTitle activeMenu={"Hose Dash Size"} motherMenu={"Home"} />
      <div className="row">
        {/* SECTION 1ST */}
              ...
              <div>
                <div className="mb-3 row">
                  <div className="col-sm-6 col-xl-3">
                    <label className=" col-form-label">Size <small style={{ color: "grey" }} >(In Inch)</small></label>
                    <input
                      name="size"
                      value={formData?.size}
                   ...
                   <div className="col-sm-6 col-xl-3">
                     <label className=" col-form-label">Dash Code</label>
                     <input
                       name="dash_code"
                       value={formData.dash_code}
                       onChange={handleChange}
                       type="text"
                       className="form-control"
                       placeholder="Ex: 01"
                     />
                     {errors.dash_code && (
                       <span className="text-danger fs-12">{errors.dash_code}</span>
                     )}
                   </div>
                </div>
              </div>
      ...
      <Row>
        <Col lg={12}>
          <div className="card">
            <div className="card-header">
              <h4 className="card-title">Hose Dash Size List</h4>
              ...
              <div className="table-responsive">
                <div id="holidayList" className="dataTables_wrapper no-footer">
                  ...
                  >
                    ...
                    <tbody>
                      {hoseDashSizeList?.hoseDashSizes?.map((data, ind) => (
                        <tr key={ind}>
                          <td>
                            {/* <strong>{ind + 1}</strong> */}
                            <strong>
                          ...
                          <td>
                            <button
                              ...
                              onClick={() => handleEditHoseDashSize(data?._id)}>
                  ...
                  <div>
                    ...
                    <div className="d-sm-flex text-center justify-content-end align-items-center mt-3">
                      <div className="pagination-container">
                        <ReactPaginate
                          pageCount={Math.ceil(
                            hoseDashSizeList?.totalHoseDashSizes / hoseDashSizeList?.rowsPerPage
                          )}
